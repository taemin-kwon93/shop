...
<tr>
	<td colspan=2 align=center>
		...
		<tr align=center height=20>
			...
			<td style=font-family:Tahoma;font-szie:8pt;><%= order.getORDER_DATE() %></td>
			<td style=font-family:Tahoma;font-szie:8pt;>
			<a href="./AdminOrderDetail.ao?num=<%=order.getORDER_NUM()%>">
			Modify</a> /
			<a href="./AdminOrderDelete.ao?num=<%=order.getORDER_NUM()%>"onclick="return confirm('삭제하시겠습니까?')">Delete</a>
			</td>
		</tr>
		<tr>
			<td style="background-color:#F0F0F0; height:1px;" colspan=6>
		...
		<tr align=center height=20>
			<td colspan=7 style=font-family:Tahoma;font-size:10pt;>
				...
				<%for(int a = startpage; a <= endpage; a++){ 
					if(a==nowpage){  %>[<%=a%>]
				<%	}else{ %>
					<a href="./AdminOrderList.ao?page=<%=a%>">[<%=a%>]</a>&nbsp;
				<%	} %>	
			<%	} %>
